...
export const parentCleanupHookFactory = ({
    ...
    const cleanupHook = async ({ req, id, doc }) => {
        ...
        for (let targetId of value) {
            ...
            console.log("Found targetDocument[target_field]: ", targetDocument[target_field])

            // get the current backrefs
            const prevReferences = targetDocument[target_field].map((ref) => ref.id ? ref.id : (ref.value.id ? ref.value.id : ref.value));

            console.log("prevReferences", prevReferences)

            let updatedReferenceIds = [];
            updatedReferenceIds = prevReferences.filter(
                (ref) => {
